...
btns.forEach((btn) => btn.addEventListener("click", function (e) {
    ...
    // getProducts(products);
    // when i click in increase btn
    //  check the btn id if it is increase
...
function emptyCart() {
    ...
    return document.getElementById('cart-container').innerHTML = `<div class="empty-container">
                                                                    <h2>Cart is Empty  </h2>
...
function amountToPay(data) {
    ...
    const total = data.reduce((acc, curr) => acc + Number(curr.price) * curr.qty , 0);
      if(total == 0){
        emptyCart();
    ...
    // else  totalPrice.classList.remove('active') 
